...
def test_parse_pluto_metadata(pluto_xdmf_file):
    file = pluto_xdmf_file
...
def test_pluto_units(pluto_xdmf_file):
    file = pluto_xdmf_file
    ds = yt.load(file["path"])
...
def test_pluto_complete_units_override(pluto_xdmf_file):
    su = SAMPLE_UNITS
    ...
    for comb in unit_combs:
        ...
        for unit in comb:
            uo[unit] = su[unit]
        if set(uo) in PlutoXdmfDataset.invalid_unit_combinations:
            continue
        ds = yt.load(
            pluto_xdmf_file["path"],
            geometry=pluto_xdmf_file["geometry"],
            units_override=uo,
        )
...
def test_pluto_one_unit_override(pluto_xdmf_file):
    file = pluto_xdmf_file
...
def test_pluto_two_units_override(pluto_xdmf_file):
    file = pluto_xdmf_file
...
def test_pluto_invalid_units_override(pluto_xdmf_file):
    for uo in PlutoXdmfDataset.invalid_unit_combinations:
...
def test_pluto_temperature_unit_override(pluto_xdmf_file):
    with pytest.raises(
        ValueError,
        match=(
...
def test_pluto_over_units_override(pluto_xdmf_file):
    with pytest.raises(
        ValueError,
        match=(
            re.escape(
...
def test_pluto_wrong_definitions_header(pluto_xdmf_file):
    with pytest.raises(
        FileNotFoundError,
        match=(r".*No such file or directory: '.*definitions2\.h'"),
    ):
        yt.load(pluto_xdmf_file["path"], definitions_header="definitions2.h")


def test_code_time(xdmf_file_with_units):
    ds = yt.load(xdmf_file_with_units["path"])
...
def test_data_access(xdmf_file):
    file = xdmf_file
...
def test_load_user_passed_geometry(xdmf_file):
    file = xdmf_file
    ds = yt.load(file["path"], geometry=file["geometry"])
    assert ds.geometry == file["geometry"]
...
def test_load_missing_geometry_metadata(xdmf_file_no_geom):
    with pytest.raises(
        ValueError,
        match=(
        ...
        yt.load(xdmf_file_no_geom["path"])


def test_load_parseable_geometry_metadata(xdmf_file_with_geom):
    file = xdmf_file_with_geom
    ds = yt.load(file["path"])
    assert ds.geometry == file["geometry"]


def test_derived_field(xdmf_file):
    file = xdmf_file
    ds = yt.load(file["path"], geometry=file["geometry"])
